...
1. **Docker Desktop** — скачайте и установите с [официального сайта](https://www.docker.com/products/docker-desktop/). Программа должна быть запущена (иконка кита в панели задач должна гореть зелёным).
...
Установка PHP и JS библиотек запускается напрямую внутри изолированной среды. Чтобы защитить сборку от сетевых обрывов и бесконечных таймаутов, Composer принудительно переводится в режим Git-исходников, а лимит ожидания операций сокращается до **34 секунд** (вместо дефолтных 5 минут):
```bash
...
# 2. Сокращаем время ожидания зависших операций жестко до 34 секунд!
...
### 5. Оживляем веб-сервер, генерируем ключи и миграции
Поскольку при первом старте контейнеры веб-сервера Nginx и сокетов Reverb упали из-за отсутствия файлов ядра, а Laravel требует папки кэша шаблонов, мы автоматически инициализируем структуру, запускаем порты, генерируем ключи безопасности и создаем таблицы базы данных:
```bash
# 1. Автоматически создаем недостающие папки системного кэша и выставляем права
docker exec -it micro_laravel mkdir -p storage/framework/sessions storage/framework/views storage/framework/cache
docker exec -it micro_laravel chmod -R 777 storage bootstrap/cache

# 2. Перезапускаем веб-сервер Nginx и сокеты Reverb (они войдут в строй за 1 секунду)
docker-compose restart nginx reverb

# 3. Генерируем уникальный секретный ключ безопасности приложения
