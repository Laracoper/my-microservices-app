...
Проект полностью автоматизирован и поднимается на любом компьютере с использованием **Docker-first подхода**. Вам **не требуется** устанавливать локально на хостовую машину PHP, Composer, Node.js или Python — все зависимости разворачиваются изолированно внутри контейнеров.
...
В папке `src/laravel-app/` создайте файл `.env` на основе `.env.example` и убедитесь, что в режиме релиза выставлены следующие параметры безопасности:
```ini
APP_ENV=production
APP_DEBUG=false
...
```
*Docker автоматически соберет PHP, скачает чистый образ Python, настроит зеркала репозиториев (Aliyun) и поднимет веб-сервер Nginx на порту `8087`.*

### 4. Контейнерная установка пакетов (Composer и NPM)
Установка PHP и JS библиотек запускается напрямую внутри изолированной среды с помощью встроенных в контейнер менеджеров пакетов:
```bash
# Устанавливаем PHP зависимости через контейнерный Composer
docker exec -it micro_laravel composer install

# Компилируем фронтенд ассеты Tailwind v4 "намертво" для релиза
docker exec -it micro_laravel npm run build
```

### 5. Скачивание ИИ-модели (Выполняется один раз)
...
### 6. Применение миграций базы данных и очистка кэша
```bash
docker exec -it micro_laravel php artisan migrate
docker exec -it micro_laravel php artisan optimize:clear
```

Система полностью готова к работе! Откройте в браузере: **`http://localhost:8087`**

---
Разработано специально в качестве демонстрационного кейса микросервисной архитектуры корпоративного уровня.
