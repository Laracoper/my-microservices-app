...
Поскольку при первом старте контейнеры веб-сервера Nginx и сокетов Reverb упали из-за отсутствия файлов ядра, их необходимо принудительно перезапустить. Затем генерируются ключи безопасности и создается структура таблиц базы данных:
```bash
# 1. Перезапускаем веб-сервер Nginx и сокеты Reverb (они войдут в строй за 1 секунду)
...
# 3. Автоматически создаем все таблицы внутри базы данных MySQL
docker exec -it micro_laravel php artisan migrate

# 4. Сбрасываем кэш настроек фреймворка
